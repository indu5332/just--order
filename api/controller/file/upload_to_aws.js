...
let uploadFile = async (req, res, next) => {
  try {
    let path = await fs.createReadStream(
      config.imagePath + "/" + req.body.file
    );
    s3.upload(
      {
        Bucket: "justorder",
        Body: path,
        Key: `restaurant/${req.decoded.id}/${req.body.file}`,
      },
      (error, data) => {
        if (error) {
          console.log(error);
...
let updateDataBase = async (req, res, next) => {
  try {
    let getLastImage = await restaurant.findById({
    ...
    let updateRes = await restaurant.updateOne(
      { _id: mongoose.Types.ObjectId(req.decoded._id) },
      { $set: { imageurl: req.data.key } }
    );
...
module.exports = [uploadFile, updateDataBase];
